...
# ---------------------------------------------------------------------------
# Kernel ISA dump (memory-ordering invariants)
# ---------------------------------------------------------------------------

#: Generated TU whose only job is to emit the shared kernels so the
#: memory-ordering checks in test_kernel.py can inspect the real instruction
#: stream. It is a bare include on purpose: the probe must compile on any tree,
#: including one that predates ``lds_barrier()``, so a missing drain shows up
#: as a failed check instead of a compile error.
KERNEL_ASM_DIR = paths.BUILD_DIR / "kernel_asm"
KERNEL_ASM_PROBE = KERNEL_ASM_DIR / "probe.cpp"
KERNEL_ASM_DUMP = KERNEL_ASM_DIR / "hip_kernels_gfx1100.s"

_KERNEL_ASM_PROBE_SOURCE = """\
// Generated by tests/correctness/harness/build.py -- do not edit.
//
// Cross-wave LDS visibility is a property of the emitted code, not of the C++
// source: a barrier with an outstanding LDS store still races, whichever
// barrier spelling produced it.
#include "hip_kernels.h"
"""


def build_kernel_asm(*, force: bool = False, no_build: bool = False) -> Path:
    """Compile ``src/common/hip_kernels.h`` to gfx1100 assembly.

    Needs ``hipcc`` but no GPU. The compile flags match the plugin build (the
    primary artifact), so the checked code is the code that ships.
    """
    deps = [paths.COMMON_SOURCES / "hip_kernels.h", KERNEL_ASM_PROBE]

    def build() -> None:
        KERNEL_ASM_DIR.mkdir(parents=True, exist_ok=True)
        KERNEL_ASM_PROBE.write_text(_KERNEL_ASM_PROBE_SOURCE)
        print(f"[correctness] emitting kernel ISA -> {KERNEL_ASM_DUMP.name}")
        run_command(
            [
                "hipcc", "--offload-arch=gfx1100", "-std=c++17", "-O3", "-ffast-math",
                "-S", "-o", str(KERNEL_ASM_DUMP),
                f"-I{paths.COMMON_SOURCES}",
                str(KERNEL_ASM_PROBE),
            ],
            cwd=paths.REPO_ROOT,
            what="kernel ISA emission",
        )

    return _ensure(KERNEL_ASM_DUMP, deps, build,
                   what="kernel ISA dump", force=force, no_build=no_build)
