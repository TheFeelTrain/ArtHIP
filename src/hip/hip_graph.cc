...
common::Status HipGraph::EnsureBuilt(const std::vector<int64_t>& input_shape) {
  ...
  for (size_t oi = 0; oi < ops_.size(); ++oi) {
    ...
    alloc_tensor(op.in0, in_shape, nullptr, 0, producer);
    if (!op.in1.empty() && !weights_.count(op.in1)) {
      alloc_tensor(op.in1, shapes[op.in1], nullptr, 0, producer);
    }
    if (op.do_add && !op.add_input.empty() && !weights_.count(op.add_input)) {
  ...
  for (const auto& op : ops_) {
    ...
    switch (op.type) {
      case OpType::Conv: {
        ...
        // The k-group pair is the fast dispatch dimension (both waves of a
        // tile region run back-to-back and share the strip via L2).
...
common::Status HipGraph::Run(const void* input_data, size_t input_bytes,
  ...
  hipMemcpyAsync(output_data, buffers_[output_buffer_].ptr, output_bytes_, hipMemcpyDeviceToHost, stream);
  ...
  if (output_is_fp32_) {
    const uint16_t* src = static_cast<const uint16_t*>(output_data);
    float* dst = static_cast<float*>(output_data);
    // Copy back via the pinned staging to avoid an aliased conversion.
    memcpy(output_staging_, output_data, output_bytes_);
    for (size_t i = 0; i < output_bytes_ / 2; ++i) {
